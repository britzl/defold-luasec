...
# define HWAES_cbc_encrypt2 aes_p8_cbc_encrypt
...
# define HWAES_cbc_encrypt2 aes_fx_cbc_encrypt
...
#  define HWAES_cbc_encrypt2 aes_v8_cbc_encrypt
...
void HWAES_decrypt(const unsigned char *in, unsigned char *out,
                   const AES_KEY *key);
void HWAES_cbc_encrypt2(const unsigned char *in, unsigned char *out,
                       size_t length, const AES_KEY *key,
                        ...
                        const unsigned char *iv, int enc)
{
    ...
    if ((mode == EVP_CIPH_ECB_MODE || mode == EVP_CIPH_CBC_MODE)
        ...
        if (HWAES_CAPABLE) {
            ...
            dat->stream.cbc = NULL;
# ifdef HWAES_cbc_encrypt2
            if (mode == EVP_CIPH_CBC_MODE)
                dat->stream.cbc = (cbc128_f) HWAES_cbc_encrypt2;
# endif
        } else
#endif
#ifdef BSAES_CAPABLE
...
#endif
        {
            ...
            dat->block = (block128_f) AES_decrypt;
            dat->stream.cbc = mode == EVP_CIPH_CBC_MODE ?
                (cbc128_f) AES_cbc_encrypt2 : NULL;
        }
    } else
#ifdef HWAES_CAPABLE
    if (HWAES_CAPABLE) {
